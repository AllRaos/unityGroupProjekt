...
{
    public RectTransform hookPointer;     // Стрілка-гачок
    public RectTransform sharkObject;
    public Image attackIndicator;         // Куди нападає акула
    public Slider sharkStaminaBar;
    public Slider lineDurabilityBar;

    public float hookRotationSpeed = 180f; // градусів за секунду
    public float hookInertia = 0.1f;      // затримка в обертанні
    public float sharkAttackDistance = 200f; // Відстань, з якої акула починає атаку
    ...
    private float attackTolerance = 25f; // градусів допуску
    ...
    void UpdateAttack()
    {
        ...
        if (timeToNextAttack <= 0f)
        {
            ...
            // Запускаємо корутину для анімації атаки
    ...
    {
        // Показуємо індикатор на 0.8 сек
        yield return new WaitForSeconds(0.8f);
        attackIndicator.gameObject.SetActive(false);

        // Показуємо акулу на початковій позиції
        ...
        sharkObject.localRotation = Quaternion.Euler(0, 0, attackAngle); // Орієнтуємо акулу

        // Анімація руху акули до центра
        float elapsed = 0f;
        Vector2 targetPos = Vector2.zero; // Центр
        ...
        // Після атаки перевіряємо результат
        ResolveAttack();
        sharkObject.gameObject.SetActive(false); // Ховаємо акулу
    }

    Vector2 CalculateSharkStartPosition(float angle)
    {
        // Конвертуємо кут у радіани та обчислюємо позицію на колі
